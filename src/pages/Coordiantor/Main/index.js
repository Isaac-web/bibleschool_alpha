...
const Index = () => {
  ...
  const handleSaveChanges = () => {
    if (!currentModule.file && !currentModule.fileUri)
      return notify("Please supply a file to this module.");
    ...
    dispatch(saveChanges(currentModule._id, currentModule, notify));
  };
          ...
          <Box style={{ overflow: "auto", height: "100%" }}>
            <Box>
              ...
              <QuestionDialog
                ...
                onNewQuestion={handleAddQuestion}
              />
              <Container maxWidth="sm" className={classes.container}>
                <FileDropZone fullWidth onChange={handleFileChange} />
                {currentModule.fileUri && <Box>{currentModule.fileUri}</Box>}

                {currentModule.file && (
                  <FileListItem file={currentModule.file} />
                )}
